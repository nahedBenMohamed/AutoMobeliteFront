...
const EditUser = ({id}) => {
    ...
    useEffect(() => {
        if (id) {
            axios
                .get("/api/super-admin/manage-user/user?id=" + id, { withCredentials: true })
                .then((response) => {
    ...
    async function saveAdmin(ev) {
        ...
        if (!name || !firstname || !email || !telephone || !numPermis || !address || !city) {
    ...
    async function uploadImage(ev) {
        ...
        if (files?.length > 0) {
            ...
            data.append("id", id);
            try {
                const res = await axios.post("/api/super-admin/manage-user/upload", data);
    ...
    async function deleteImage() {
        ...
        try {
            await axios.delete(`/api/super-admin/manage-user/delete?id=${id}`, {withCredentials: true});
