...
const AddAgence = ({ id }) => {
    ...
    async function saveAgence(ev) {
        ...
        if (!name || !telephone || !email || !address || !responsibleId) {
        ...
        const data = { name, address, email, telephone, responsibleId };
        try {
            if (id) {
                // Update
                await axios.put("/api/super-admin/agence/"+id, data);
            } else {
                // Create
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        value={responsibleId}
                                        onChange={(e) => setResponsibleId(parseInt(e.target.value))}
                                        className="pl-10 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"
                                    />
                                </div>
                            </div>
