...
const SuperAdminParkingForm = ({ id }) => {
  ...
  useEffect(() => {
    if (id) {
      axios
        .get("/api/super-admin/parking-agence/parking?id=" + id, {
  ...
  async function saveParking(ev) {
    ...
    try {
      if (id) {
        // Update
        await axios.put("/api/super-admin/parking-agence/parking/", {
          ...data,
          id,
        });
      } else {
    ...
    if (files?.length > 0) {
      ...
      try {
        const res = await axios.post(
          "/api/super-admin/parking-agence/upload",
          data
        );
  ...
  async function deleteImage() {
    ...
    try {
      await axios.delete(`/api/super-admin/parking-agence/delete?id=${id}`, {
        withCredentials: true,
      });
