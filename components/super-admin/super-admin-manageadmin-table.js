...
const SuperAdminManageTable = () => {
  ...
  const handleDeleteCar = async () => {
    try {
      await axios.delete(
        `/api/super-admin/manage-admin/admin?id=${adminToDelete.id}`,
        { withCredentials: true }
      );
      const updatedAdmin = admin.filter(
                  ...
                  .map((admin) => (
                    <tr key={admin.id}>
                      ...
                      <td className="text-center">
                        {admin.Agency ? admin.Agency.name : "N/A"}
                      </td>
                      <td className="px-6 py-4 justify-center flex space-x-2">
                        <button
                          onClick={() => goEditAdmin(admin.id)}
                ...
                {currentPage !== Math.ceil(admin.length / adminPerPage) && (
                  <>
                    ...
                    <li>
                      <button
                        onClick={() =>
                          setCurrentPage(Math.ceil(admin.length / adminPerPage))
                        }
                        className="bg-gray-200 hover:bg-gray-300 text-gray-600 font-bold py-2 px-4 mx-1 rounded"
                      >
                        Last
