...
// Connect to the socket server after you have received the agencyId
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.post(`/api/client/reservation/`, { ...data });
            if (response.status === 201) {
                ...
                socket.emit('newReservation', rentalData);
                console.log('Emitted newReservation:', rentalData);
                setTimeout(() => {
...
// Make sure to disconnect from the socket server when the component is unmounted
    useEffect(() => {
        return () => {
            if (socket) {
                socket.disconnect();
    ...
    }, []); // This useEffect will only run once, when the component is unmounted
