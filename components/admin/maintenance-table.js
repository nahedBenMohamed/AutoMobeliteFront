            ...
            <main className="flex-grow">
                <div className="bg-none rounded-lg p-4">
                    <div className="flex justify-between items-center">
                        <Link href="/admin/dashboard/cars/" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                            Add Car Maintenance
                        </Link>
                        <input
                            type="text"
                    ...
                    <div className="bg-white shadow-md rounded-lg overflow-x-auto">
                        <div className="inline-block min-w-full overflow-hidden">
                            <table className="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody>
                                {currentMaintenance
                                    /*.filter(parking => car.name.toLowerCase().includes(search.toLowerCase()) ||
                                        parking.city.toLowerCase().includes(search.toLowerCase())
                                    )*/
                                    .map((maintenance) => (
                                        <tr key={maintenance.id}>
                                            <td className="px-6 py-4 whitespace-nowrap">{maintenance.car.brand}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">{maintenance.car.model}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">{new Date(maintenance.startDate).toLocaleDateString()}</td>
                                            <td className="px-6 py-4 whitespace-nowrap">{new Date(maintenance.endDate).toLocaleDateString()}</td>
                                            <td className="px-6 py-4 flex justify-center">
                                                <Link href={`/admin/dashboard/maintenance/edit/${maintenance.id}`} className="text-blue-500 hover:text-blue-700 mx-1">
                                                    <FiEdit size={18} />
                                                </Link>
                                                {/*<button
                    ...
                    <div className="flex justify-center mt-4">
                        {maintenance.length > maintenancePerPage && (
                            <ul className="flex items-center">
                                {Array.from({ length: Math.ceil(maintenance.length / maintenancePerPage) }).map((_, index) => (
                                    <li key={index}>
                                        <button
                                            className={`${
                                                currentPage === index + 1
                                ...
                                <div className="flex flex-col sm:flex-row">
                                    ...
                                    <div className="w-full sm:w-1/2 mx-4 mt-4 sm:mt-0">
                                        <div className="rounded-lg p-4 space-y-4">
                                            <h2 className="text-xl sm:text-2xl font-bold">{selectedCar.name}</h2>
                                            <p><span className="font-bold">Address:</span>&nbsp;{selectedCar.address}</p>
                                            <p><span className="font-bold">City:</span>&nbsp;{selectedCar.city}</p>
                                            <p><span className="font-bold">Agency Name:</span>&nbsp;{selectedCar.Agency?.name}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
