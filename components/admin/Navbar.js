...
export default function Navbar({ session }) {
  ...
  const markNotificationAsRead = async (notification) => {
    const response = await axios.post(
      `http://localhost:8000/notification/${notification.id}/read`
    );
    ...
    if (updatedNotification.reservationId) {
      await router.push(
        `/admin/dashboard/reservations/details/${updatedNotification.reservationId}`
      );
    }
  };
    ...
    <nav className="navbar flex justify-between">
      ...
      <div className="flex items-center">
        <Menu as="div" className="relative inline-block text-left">
          ...
          >
            <Menu.Items className="absolute right-0 w-80 mt-2 origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg outline-none">
