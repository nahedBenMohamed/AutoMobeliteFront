...
export default function Sidebar({ session }) {
  ...
  useEffect(() => {
    if (agencyId) {
      axios
        .get(`/api/super-admin/manage-agence/agence?id=${agencyId}`, {
          withCredentials: true,
        })
        .then((response) => {
  ...
  return (
    <section id="sidebar">
      ...
      <ul className="side-menu top">
        ...
        >
          ...
          {(isHovered2 || isLinkClicked2) && (
            <ul className="absolute top-8 left-0 w-48 rounded-md py-2">
              <li>
                <Link
                  href="/admin/dashboard/agence/details/"
        ...
        <li
          className={pathname === "/admin/dashboard/messages" ? "active" : ""}
        >
        ...
        <li
          className={
            pathname === "/admin/dashboard/maintenance" ? "active" : ""
          }
        >
