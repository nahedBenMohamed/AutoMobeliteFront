...
export default async function handle(req, res) {
  ...
  try {
    ...
    if (method === "PUT") {
      const { id, name, firstname, email, image, agencyName, isActive } =
        req.body;
      ...
      if (agencyName) {
        ...
        if (agencyResponsibleId && agencyResponsibleId !== admin.id) {
          return res
            .status(400)
            .json({
              message: "The specified agency already has a responsible.",
            });
        }
      } else {
    ...
    if (method === "DELETE") {
      ...
      if (agency) {
        return res
          .status(400)
          .json({
            message: "Cannot delete admin. Admin is responsible for an agency.",
          });
      }
