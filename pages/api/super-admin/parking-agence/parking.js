  ...
  try {
    if (method === "POST") {
      ...
      if (!agency) {
        return res
          .status(400)
          .json({ message: "No agency matching this name." });
      }
      ...
      return res
        .status(200)
        .json({ message: "Parking added successfully.", parking: newParking });
    } else if (method === "GET") {
      // Handle GET method
      const { id } = req.query;
      ...
      if (
        existingParking.Agency &&
        existingParking.Agency.id !== existingAgency.id
      ) {
        return res
          .status(400)
          .json({ message: "This parking is already assigned to an agency." });
      }
