...
export default async (req, res) => {
    ...
    try {
        if (req.method === 'POST') {
            ...
            const resetToken = jwt.sign({ userId: existingUser.id }, process.env.JWT_SECRET, {
                expiresIn: '1h',
            });
        ...
        <div style="background-color: #f5f5f5; font-family: 'Arial', sans-serif; width: 100%; display: flex; justify-content: center;">
            <div style="max-width: 600px; width: 100%; margin: 0 auto;">
                <div style="background-color: #fff; padding: 40px;">
                    ...
                    <p style="margin-bottom: 20px;">Hello ${existingUser.name},</p>
                    <p>You have requested a password reset for your account on the <strong>AUTOMOBELITE</strong> application.</p>
                    <p>Click the button below to reset your password:</p>
                    <div style="text-align: center; margin-top: 30px;">
                        <a href="${process.env.BASE_URL}/reset-password?token=${resetToken}" style="background-color: #4a90e2; color: #fff; padding: 12px 24px; text-decoration: none; border-radius: 4px; font-weight: bold; display: inline-block;">Reset Password</a>
                    </div>
    ...
    `,
            ...
            return res.status(200).json({ clientUpdate: 'password changed successfully' });
        } else {
